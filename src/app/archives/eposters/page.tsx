				...
				for (const congress of pastCongresses) {
					...
					try {
						...
						if (response.ok) {
							...
							// Include common document file types, not just PDFs
						...
						<div className="space-y-12">
							{availableYears
								...
								.map((year) => {
									...
									// Group posters by congress/event
									const postersByEvent = yearPosters.reduce((acc, poster) => {
										if (!acc[poster.congressId]) {
											acc[poster.congressId] = {
												title: poster.congressTitle,
												posters: [],
											};
										}
										acc[poster.congressId].posters.push(poster);
										return acc;
									}, {} as Record<string, { title: string; posters: EPoster[] }>);
									...
									return (
										<div key={year} className="space-y-8">
											...
											{/* Map through each event/congress */}
											{Object.entries(postersByEvent).map(
												([congressId, eventData], eventIndex, eventEntries) => (
													<div key={congressId} className="space-y-4">
														<div className="flex items-center justify-between">
															<h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 pl-2 border-l-4 border-blue-500 dark:border-blue-400 flex items-center">
																{eventData.title}
																<span className="ml-3 text-sm font-normal bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 py-1 px-2 rounded-full">
																	{eventData.posters.length}{' '}
																	{eventData.posters.length === 1
																		? t('archives.document', 'document')
																		: t('archives.documents', 'documents')}
																</span>
															</h3>
															<Link
																href={`/archives/events/${congressId}`}
																className="text-sm font-medium text-blue-600 dark:text-blue-400 flex items-center hover:underline"
															>
																{t('archives.viewEvent', 'View Event')}
																				...
																				<div className="flex items-center">
																					{(() => {
																						...
																						switch (fileExt) {
																							...
																							case '.xlsx':
																								return (
																									<div className="w-10 h-10 flex items-center justify-center bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded">
																										<FileText className="w-6 h-6" />
																									</div>
																								);
																							default:
																								return (
																									<div className="w-10 h-10 flex items-center justify-center bg-gray-100 dark:bg-gray-900/30 text-gray-600 dark:text-gray-400 rounded">
																										<FileText className="w-6 h-6" />
																									</div>
																								);
																						}
														...
														{/* Add a divider between events except after the last one */}
														{eventIndex < eventEntries.length - 1 && (
